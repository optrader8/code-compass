...
class CodeAnalyzer {
    ...
    calculateCognitiveComplexity(ast) {
        ...
        return Math.max(1, Math.round(baseComplexity * 0.8 +
    ...
    calculateHalsteadMetrics(ast) {
        ...
        for (const op of operators) {
            const regex = new RegExp(`\\${op.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g');
            const matches = content.match(regex);
            if (matches) {
        ...
        const uniqueOperators = new Set(operators.filter(op => content.includes(op))).size;
    ...
    async analyzeDependencies(ast) {
        ...
        if (ast.language === ast_1.Language.TypeScript ||
            ast.language === ast_1.Language.JavaScript) {
    ...
    async calculateMetrics(ast) {
        ...
        const avgFunctionSize = functionCount > 0 ? linesOfCode / functionCount : linesOfCode;
        const avgClassSize = classCount > 0 ? linesOfCode / classCount : linesOfCode;
        return {
            linesOfCode,
    ...
    countLinesWithoutComments(content) {
    ...
    async analyzeSymbol(symbol, context) {
    ...
    async calculateComplexityForSymbol(symbol) {
    ...
    async extractDocumentation(ast) {
